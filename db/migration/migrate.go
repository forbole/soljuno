...
func Up(db db.ExcecutorDb) error {
	_, err := db.Exec(`
		DELETE FROM token_delegation WHERE NOT EXISTS (
			SELECT 1 FROM token_account WHERE token_account.address = token_delegation.source_address
		);
		DELETE FROM token_delegation WHERE NOT EXISTS (
			SELECT 1 FROM token_account WHERE token_account.address = token_delegation.delegate_address
		);
