...
func (db *Database) GetLastPruned() (uint64, error) {
	...
	err := db.Sqlx.QueryRow(`SELECT coalesce(MAX(last_pruned_slot),0) FROM pruning LIMIT 1;`).Scan(&lastPrunedHeight)
	return lastPrunedHeight, err
}

// StoreLastPruned implements db.PruningDb
func (db *Database) StoreLastPruned(slot uint64) error {
	_, err := db.Sqlx.Exec(`DELETE FROM pruning`)
	if err != nil {
		return err
	}

	_, err = db.Sqlx.Exec(`INSERT INTO pruning (last_pruned_slot) VALUES ($1)`, slot)
	return err
}
